...
- The keys correspond ONE-TO-ONE and IN ORDER to the script's keys (each "the [ordinal] key is..." section). 'title' MUST reuse the key's own opening words: when the script says 'the third key is X', the title is '3, X' with X kept nearly verbatim - trimmed to fit, never re-phrased into your own words.
- Premise, affirmation, cta, stepping_back and closing are lifted from the script's own lines, compressed but recognizably ITS words: stepping_back is compressed FROM the script's actual recap sentences (after 'here is the whole picture'), closing uses the script's actual closing lines.
- TRACEABILITY RULE: every phrase you output must be traceable to the script - if the script did not say it, it does not go in the map.
